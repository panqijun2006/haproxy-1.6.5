...
static int raw_sock_to_buf(struct connection *conn, struct buffer *buf, int count)
{
    ...
    HA_DBG("in raw_sock_to_buf count %d\n", count);
	if (!conn_ctrl_ready(conn))
		return 0;
	...
	while (count > 0) {
		...
		if (try > count)
			try = count;
        HA_DBG("try to recv try %d, fd %d\n", try, conn->t.sock.fd);
		//ret = recv(conn->t.sock.fd, bi_end(buf), try, 0);
        ret = mtcp_read(g_mctx, conn->t.sock.fd, bi_end(buf), try); 
        HA_DBG("try to recv ret %d\n", ret);
		if (ret > 0) {
			buf->i += ret;
			done += ret;
			if (ret < try) {
            ...
            HA_DBG("errno == EAGAIN || errno == ENOTCONN\n");
	...
	if (unlikely(conn->flags & CO_FL_WAIT_L4_CONN) && done)
		conn->flags &= ~CO_FL_WAIT_L4_CONN;
    HA_DBG("raw_sock_to_buf done\n");
	return done;

 read0:
    HA_DBG("conn_sock_read0\n");
	conn_sock_read0(conn);
	conn->flags &= ~CO_FL_WAIT_L4_CONN;
...
static int raw_sock_from_buf(struct connection *conn, struct buffer *buf, int flags)
{
    ...
    HA_DBG("in raw_sock_from_buf fd %d\n", conn->t.sock.fd);
	if (!conn_ctrl_ready(conn))
		return 0;
	...
	while (buf->o) {
		...
		if (try < buf->o || flags & CO_SFL_MSG_MORE)
			send_flag |= MSG_MORE;
        HA_DBG("try mtcp_write, fd %d, len %d\n", conn->t.sock.fd, try);
		//ret = send(conn->t.sock.fd, bo_ptr(buf), try, send_flag);
		ret = mtcp_write(g_mctx, conn->t.sock.fd, bo_ptr(buf), try);
        HA_DBG("after mtcp_write, fd %d, len %d ret %d\n", conn->t.sock.fd, try, ret);
