...
struct stream *stream_new(struct session *sess, struct task *t, enum obj_type *origin)
{
	...
	s->task = t;
	t->process = process_stream;   //b)	Stream关联任务task，task的回调为process_stream
	t->context = s;
	...
	if (conn)
		conn_data_want_recv(conn);  //f)	调用conn_data_want_recv将连接fd添加到事件侦听列表中
	else if (appctx)
    ...
    HA_DBG("before task_wakeup(t, TASK_WOKEN_INIT);\n");
	return s;
...
struct task *process_stream(struct task *t)
{
	...
	if (si_b->state == SI_ST_CON) {
		 ...
		 */
		HA_DBG("in si_b->state == SI_ST_CON\n");
 ...
 resync_stream_interface:
	...
	if (unlikely(si_f->state == SI_ST_DIS)) {
        ...
        HA_DBG("nothing special to be done on client side\n");
	}
