...
/* private data */
//static struct epoll_event *epoll_events;  modify by panqijun for mtcp
static struct mtcp_epoll_event *epoll_events;
static int epoll_fd;

/* This structure may be used for any purpose. Warning! do not use it in
 * recursive functions !
 */
//static struct epoll_event ev;   modify by panqijun for mtcp 
static struct mtcp_epoll_event ev; 

// mtcp context add by panqijun
mctx_t g_mctx;
extern int g_cpu_id;
extern in_addr_t g_ser_addr;
#define IP_RANGE 1
...
{   
    HA_DBG("in __fd_clo %d cloned %d\n", fd, fdtab[fd].cloned);
	if (unlikely(fdtab[fd].cloned)) {
        //epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, &ev);
		mtcp_epoll_ctl(g_mctx, epoll_fd, EPOLL_CTL_DEL, fd, &ev);
        HA_DBG("__fd_clo try to close fd %d\n", fd);
	}
}
...
REGPRM2 static void _do_poll(struct poller *p, int exp)
{
	...
	for (updt_idx = 0; updt_idx < fd_nbupdt; updt_idx++) {
		...
		if ((eo ^ en) & FD_EV_POLLED_RW) {
            ...
            mtcp_epoll_ctl(g_mctx, epoll_fd, opcode, fd, &ev);
		}
	}
	fd_nbupdt = 0;
    ...
    status = mtcp_epoll_wait(g_mctx, epoll_fd, epoll_events, global.tune.maxpollevents, wait_time);
	...
	for (count = 0; count < status; count++) {
		...
		fd = epoll_events[count].data.sockid;
...
REGPRM1 static int _do_init(struct poller *p)
{
    ...
    printf("epoll init, affinitize cpu id %d\n", g_cpu_id);
    /* add by panqijun for mtcp context */
    mtcp_core_affinitize(g_cpu_id);
    g_mctx = mtcp_create_context(g_cpu_id);
    if (!g_mctx) {
    ...
    if (g_ser_addr != 0) {
        ret = mtcp_init_rss(g_mctx, INADDR_ANY, IP_RANGE, g_ser_addr, htons(80));
        if (ret < 0) {
    		printf("mtcp_init_rss failed, ret %d\n", ret);
    		exit(1);
    	}
    }
    
	//epoll_fd = epoll_create(global.maxsock + 1);
    epoll_fd = mtcp_epoll_create(g_mctx, global.maxsock + 1);
	if (epoll_fd < 0)
		goto fail_fd;

	ret = mtcp_setsock_nonblock(g_mctx, epoll_fd);
	if (ret < 0) {
		printf("Failed to set socket in nonblocking mode, ret %d\n", ret);
		exit(1);
	}
	...
	epoll_events = (struct epoll_event*)
		...
		*/
	epoll_events = (struct mtcp_epoll_event*)
		calloc(1, sizeof(struct epoll_event) * global.tune.maxpollevents);
