...
int my_socketat(const struct netns_entry *ns, int domain, int type, int protocol)
{
	...
	if (default_namespace >= 0 && ns && setns(ns->fd, CLONE_NEWNET) == -1)
		return -1;
#endif
    HA_DBG("in my_socketat\n");
	//sock = socket(domain, type, protocol);
    sock = mtcp_socket(g_mctx, domain, type, protocol);
    //mtcp_setsock_nonblock(g_mctx, sock);
    HA_DBG("in my_socketat mtcp_socket sock %d\n", sock);
    
#ifdef CONFIG_HAP_NS
	if (default_namespace >= 0 && ns && setns(default_namespace, CLONE_NEWNET) == -1) {
		//close(sock);
        mtcp_close(g_mctx, sock);
		return -1;
	}
#endif
