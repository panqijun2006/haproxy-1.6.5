...
int tcp_bind_socket(int fd, int flags, struct sockaddr_storage *local, struct sockaddr_storage *remote)
{
	...
	case AF_INET:
		if (flags && ip_transp_working) {
			...
			if (0
			    ...
			    || (mtcp_setsockopt(g_mctx, fd, SOL_IP, IP_TRANSPARENT, &one, sizeof(one)) == 0)
#endif
#if defined(IP_FREEBIND)
			    || (mtcp_setsockopt(g_mctx, fd, SOL_IP, IP_FREEBIND, &one, sizeof(one)) == 0)
#endif
#if defined(IP_BINDANY)
			    || (mtcp_setsockopt(g_mctx, fd, IPPROTO_IP, IP_BINDANY, &one, sizeof(one)) == 0)
#endif
#if defined(SO_BINDANY)
			    || (mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_BINDANY, &one, sizeof(one)) == 0)
#endif
			    )
				foreign_ok = 1;
			else
	...
	case AF_INET6:
		if (flags && ip6_transp_working) {
			if (0
			    ...
			    || (mtcp_setsockopt(g_mctx, fd, SOL_IPV6, IPV6_TRANSPARENT, &one, sizeof(one)) == 0)
#endif
#if defined(IP_FREEBIND)
			    || (mtcp_setsockopt(g_mctx, fd, SOL_IP, IP_FREEBIND, &one, sizeof(one)) == 0)
#endif
#if defined(IPV6_BINDANY)
			    || (mtcp_setsockopt(g_mctx, fd, IPPROTO_IPV6, IPV6_BINDANY, &one, sizeof(one)) == 0)
#endif
#if defined(SO_BINDANY)
			    || (mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_BINDANY, &one, sizeof(one)) == 0)
#endif
	...
	mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one));
	if (foreign_ok) {
		if (is_inet_addr(&bind_addr)) {
			ret = mtcp_bind(g_mctx, fd, (struct sockaddr *)&bind_addr, get_addr_len(&bind_addr));
			if (ret < 0)
				return 2;
		}
	}
		...
		if (is_inet_addr(local)) {
			ret = mtcp_bind(g_mctx, fd, (struct sockaddr *)local, get_addr_len(local));
			if (ret < 0)
				return 1;
		}
	}
...
int tcp_connect_server(struct connection *conn, int data, int delack)
{
	...
	if (fd >= global.maxsock) {
		...
		//close(fd);
        mtcp_close(g_mctx, fd);
		conn->err_code = CO_ER_CONF_FDLIM;
		conn->flags |= CO_FL_ERROR;
		return SF_ERR_PRXCOND; /* it is a configuration limit */
	}

	if ((mtcp_setsock_nonblock(g_mctx, fd) == -1) ||
	    (mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one)) == -1)) {
	...
	if (be->options & PR_O_TCP_SRV_KA)
		mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_KEEPALIVE, &one, sizeof(one));
	...
	if (src) {
		...
		if (src->iface_name)
			mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_BINDTODEVICE, src->iface_name, src->iface_len + 1);
#endif
	...
	if (delack == 2 || ((delack || data || conn->send_proxy_ofs) && (be->options2 & PR_O2_SMARTCON)))
                mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_QUICKACK, &zero, sizeof(zero));
#endif

#ifdef TCP_USER_TIMEOUT
	/* there is not much more we can do here when it fails, it's still minor */
	if (srv && srv->tcp_ut)
		mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_USER_TIMEOUT, &srv->tcp_ut, sizeof(srv->tcp_ut));
#endif
	if (global.tune.server_sndbuf)
                mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_SNDBUF, &global.tune.server_sndbuf, sizeof(global.tune.server_sndbuf));

	if (global.tune.server_rcvbuf)
                mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_RCVBUF, &global.tune.server_rcvbuf, sizeof(global.tune.server_rcvbuf));

    // modify by panqijun
    HA_DBG("try to mtcp_connect\n");
    ret = mtcp_connect(g_mctx, fd, (struct sockaddr *)&conn->addr.to, get_addr_len(&conn->addr.to));
    HA_DBG("mtcp_connect ret %d\n", ret);
	if (ret == -1 &&
    ...
    HA_DBG("before conn_ctrl_init\n");
	conn_ctrl_init(conn);       /* registers the FD */
	fdtab[fd].linger_risk = 1;  /* close hard if needed */
    HA_DBG("before conn_sock_want_send\n");
	conn_sock_want_send(conn);  /* for connect status */
    HA_DBG("after conn_sock_want_send\n");
...
int tcp_get_dst(int fd, struct sockaddr *sa, socklen_t salen, int dir)
{
	if (dir)
		...
		if (((struct sockaddr_storage *)sa)->ss_family == AF_INET
		    && mtcp_getsockopt(g_mctx, fd, SOL_IP, SO_ORIGINAL_DST, sa, &salen) == 0)
			return 0;
#endif
		return ret;
	}
...
int tcp_bind_listener(struct listener *listener, char *errmsg, int errlen)
{
	...
	if (!ext) {
        HA_DBG("before call my_socketat\n");
		fd = my_socketat(listener->netns, listener->addr.ss_family, SOCK_STREAM, IPPROTO_TCP);   ////////´´½¨socket
	...
	if (!ext && mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) == -1) {
	...
	if (listener->options & LI_O_NOLINGER)
		mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_LINGER, &nolinger, sizeof(struct linger));
	 ...
	 */
	if (!ext)
		mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_REUSEPORT, &one, sizeof(one));
#endif
	...
	if (!ext && (listener->options & LI_O_FOREIGN)) {
		...
		case AF_INET:
			if (1
#if defined(IP_TRANSPARENT)
			    && (mtcp_setsockopt(g_mctx, fd, SOL_IP, IP_TRANSPARENT, &one, sizeof(one)) == -1)
#endif
#if defined(IP_FREEBIND)
			    && (mtcp_setsockopt(g_mctx, fd, SOL_IP, IP_FREEBIND, &one, sizeof(one)) == -1)
#endif
#if defined(IP_BINDANY)
			    && (mtcp_setsockopt(g_mctx, fd, IPPROTO_IP, IP_BINDANY, &one, sizeof(one)) == -1)
#endif
#if defined(SO_BINDANY)
			    && (mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_BINDANY, &one, sizeof(one)) == -1)
#endif
			    ) {
		...
		case AF_INET6:
			if (1
#if defined(IPV6_TRANSPARENT)
			    && (mtcp_setsockopt(g_mctx, fd, SOL_IPV6, IPV6_TRANSPARENT, &one, sizeof(one)) == -1)
#endif
#if defined(IP_FREEBIND)
			    && (mtcp_setsockopt(g_mctx, fd, SOL_IP, IP_FREEBIND, &one, sizeof(one)) == -1)
#endif
#if defined(IPV6_BINDANY)
			    && (mtcp_setsockopt(g_mctx, fd, IPPROTO_IPV6, IPV6_BINDANY, &one, sizeof(one)) == -1)
#endif
#if defined(SO_BINDANY)
			    && (mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_BINDANY, &one, sizeof(one)) == -1)
#endif
	...
	if (!ext && listener->interface) {
		if (mtcp_setsockopt(g_mctx, fd, SOL_SOCKET, SO_BINDTODEVICE,
	...
	if (listener->maxseg > 0) {
		if (mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_MAXSEG,
	...
	if (listener->tcp_ut) {
		if (mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_USER_TIMEOUT,
	...
	if (listener->options & LI_O_DEF_ACCEPT) {
		...
		int accept_delay = 1;
		if (mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &accept_delay, sizeof(accept_delay)) == -1) {
	...
	if (listener->options & LI_O_TCP_FO) {
		...
		if (mtcp_setsockopt(g_mctx, fd, IPPROTO_TCP, TCP_FASTOPEN, &qlen, sizeof(qlen)) == -1) {
	...
	if (listener->options & LI_O_V6ONLY)
                mtcp_setsockopt(g_mctx, fd, IPPROTO_IPV6, IPV6_V6ONLY, &one, sizeof(one));
	else if (listener->options & LI_O_V4V6)
                mtcp_setsockopt(g_mctx, fd, IPPROTO_IPV6, IPV6_V6ONLY, &zero, sizeof(zero));
#endif

	if (!ext && mtcp_bind(g_mctx, fd, (struct sockaddr *)&listener->addr, listener->proto->sock_addrlen) == -1) {
		err |= ERR_RETRYABLE | ERR_ALERT;
	...
	if (mtcp_getsockopt(g_mctx, fd, SOL_SOCKET, SO_ACCEPTCONN, &ready, &ready_len) == -1)
		ready = 0;

    HA_DBG("before call mtcp_listen\n");
	if (!(ext && ready) && /* only listen if not already done by external process */
	    mtcp_listen(g_mctx, fd, listener->backlog ? listener->backlog : listener->maxconn) == -1) {
		err |= ERR_RETRYABLE | ERR_ALERT;
