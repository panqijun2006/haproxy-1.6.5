...
int session_accept_fd(struct listener *l, int cfd, struct sockaddr_storage *addr)
{
    ...
    HA_DBG("in session_accept_fd cfd %d\n", cfd);
	...
	if ((l->options & LI_O_TCP_RULES) && !tcp_exec_req_rules(sess)) {
		...
		mtcp_setsockopt(g_mctx, cfd, SOL_SOCKET, SO_LINGER, (struct linger *) &nolinger, sizeof(struct linger));
	...
	if (l->addr.ss_family == AF_INET || l->addr.ss_family == AF_INET6) {
		mtcp_setsockopt(g_mctx, cfd, IPPROTO_TCP, TCP_NODELAY, (char *) &one, sizeof(one));

		if (p->options & PR_O_TCP_CLI_KA)
			mtcp_setsockopt(g_mctx, cfd, SOL_SOCKET, SO_KEEPALIVE, (char *) &one, sizeof(one));
		...
		if (l->maxseg < 0) {
			...
			if (mtcp_getsockopt(g_mctx, cfd, IPPROTO_TCP, TCP_MAXSEG, &mss, &mss_len) == 0) {
				mss += l->maxseg; /* remember, it's < 0 */
				mtcp_setsockopt(g_mctx, cfd, IPPROTO_TCP, TCP_MAXSEG, &mss, sizeof(mss));
			}
		}
#endif
	}
	...
	if (global.tune.client_sndbuf)
		mtcp_setsockopt(g_mctx, cfd, SOL_SOCKET, SO_SNDBUF, &global.tune.client_sndbuf, sizeof(global.tune.client_sndbuf));

	if (global.tune.client_rcvbuf)
		mtcp_setsockopt(g_mctx, cfd, SOL_SOCKET, SO_RCVBUF, &global.tune.client_rcvbuf, sizeof(global.tune.client_rcvbuf));

	if (unlikely((t = task_new()) == NULL))  ///////////f)	调用task_new创建任务，任务的上下文为session本身；
		goto out_free_sess;
	...
	cli_conn->flags |= CO_FL_CONNECTED;   // h)	如果无需握手，则认为连接成功，打上标记CO_FL_CONNECTED；
