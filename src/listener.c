...
//#ifdef USE_ACCEPT4
    ...
    HA_DBG("in listener_accept fd %d\n", fd);
	...
	while (max_accept--) {
		...
		if (unlikely(p && p->feconn >= p->maxconn)) {
            HA_DBG("after max_accept 02\n");
			limit_listener(l, &p->listener_queue);
            HA_DBG("after max_accept 2\n");
			return;
		}
       HA_DBG("before mtcp_accept\n");
//#ifdef USE_ACCEPT4
#if 0
		...
		if (unlikely(accept4_broken ||
			...
			if ((cfd = mtcp_accept(g_mctx, fd, (struct sockaddr *)&addr, &laddr)) != -1) {
                HA_DBG("fd %d cfd %d mtcp_accept ok, then mtcp_setsock_nonblock\n", fd, cfd);
				//fcntl(cfd, F_SETFL, O_NONBLOCK);
				mtcp_setsock_nonblock(g_mctx, fd);
				mtcp_setsock_nonblock(g_mctx, cfd);
			}
        HA_DBG("after mtcp_accept\n");
		if (unlikely(cfd == -1)) {
            HA_DBG("unlikely(cfd == -1)\n");
			switch (errno) {
			case EAGAIN:
        ...
        ret = l->accept(l, cfd, &addr);   // µ÷ÓÃ session_accept_fd
		if (unlikely(ret <= 0)) {
