...
char *g_mtcp_cfg=NULL;
int g_cpu_id = -1;
int g_cpu_num = -1;
in_addr_t g_ser_addr = 0;
char g_ser_host[16 + 1];
...
void usage(char *name)
{
	...
	fprintf(stderr,
		...
		"D ] [ -n <maxconn> ] [-F <mtcp_cfg_file>] [-U cpu-num] [-I cpu-id] [ -N <maxpconn> ]"
		" [-B blackend-ip]\n"
...
void dpdk_mtcp_signal_handler(int signum)
{
    exit(0);
}

/* dpdk + mtcp init add by panqijun */
int dpdk_mtcp_init(void)
{
    int ret;
    struct mtcp_conf mcfg;

    mtcp_register_signal(SIGINT, dpdk_mtcp_signal_handler);

	mtcp_getconf(&mcfg);
	mcfg.num_cores = g_cpu_num; 
	mtcp_setconf(&mcfg);
    ret = mtcp_init(g_mtcp_cfg);
    if (ret) {
        printf("Failed to initialize mtcp, ret %d\n", ret);
        exit(1);
    } else {
        printf("mtcp init OK, config file: %s\n", g_mtcp_cfg);
    }
...
void init(int argc, char **argv)
{
		...
		if (**argv == '-') {
			...
			if (*flag == 'v') {
                ...
                case 'I':
                    ...
                    break;
                case 'B':
                    strncpy(g_ser_host, *argv, 16);
                    printf("Blackend server host: %s\n", g_ser_host);
                    g_ser_addr = inet_addr(g_ser_host);
                    break;
    ...
    /* add by panqiun for MTCP config file */
    if (g_mtcp_cfg == NULL)
		usage(progname);

    if (g_cpu_id == -1 || g_cpu_num <= 0)
        usage(progname);
...
void run_poll_loop()
{
	...
	while (1) {
		...
		cur_poller.poll(&cur_poller, next);  ///将fd放入缓存事件中
        //HA_DBG("cur_poller.name %s\n", cur_poller.name);  =====> epoll
		fd_process_cached_events();    //处理缓存事件（即poll中生产的事件）
		applet_run_active();
	}
}
...
int main(int argc, char **argv)
{
	...
	while (retry >= 0) {
		struct timeval w;
		err = start_proxies(retry == 0 || nb_oldpids == 0);  //////启动所有代理（即bind/listen端口）
	...
	err = protocol_bind_all(errmsg, sizeof(errmsg));  //绑定所有的协议侦听的端口
	if ((err & ~ERR_WARN) != ERR_NONE) {
	...
	if (global.mode & (MODE_DAEMON | MODE_SYSTEMD)) {
        ...
        HA_DBG("global.nbproc %d\n", global.nbproc);
		/* the father launches the required number of processes */
		for (proc = 0; proc < global.nbproc; proc++) {  //////根据配置的进程个数，fork子进程，子进程与父进程运行后续相同的代码
			ret = fork();
			if (ret < 0) {
		...
		if (proc == global.nbproc) {
			...
			exit(0); /* parent must leave */  ////////////////主进程会退出
		}
